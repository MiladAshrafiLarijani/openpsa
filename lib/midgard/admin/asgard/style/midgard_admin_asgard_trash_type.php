...
echo "<h2>";
echo sprintf(midcom::i18n()->get_string('%s trash', 'midgard.admin.asgard'), midgard_admin_asgard_plugin::get_type_label($data['type']));
echo "</h2>";
...
function midgard_admin_asgard_trash_type_show($object, $indent = 0, $prefix = '', $enable_undelete = true)
{
    ...
    if (!$url_prefix)
    {
        $url_prefix =midcom::get_context_data(MIDCOM_CONTEXT_ANCHORPREFIX);
    }
    ...
    {
        $persons[$object->metadata->revisor] = midcom::auth->get_user($object->metadata->revisor);
    }
        ...
        && count($child_types) > 0)
    {
        ...
        foreach ($child_types as $type => $children)
        {
            ...
            else
            {
                ...
                echo "{$prefix}    <tr>\n";
                echo "{$prefix}        <td class=\"label\" style=\"padding-left: {$child_indent}px\" colspan=\"5\"><a href=\"?show_children[{$object->guid}][{$type}]=1\">" . sprintf(midcom::i18n()->get_string('show %s %s children', 'midgard.admin.asgard'), count($children), midgard_admin_asgard_plugin::get_type_label($type)) . "</a></td>\n";
                echo "{$prefix}    </tr>\n";
            }
        }
...
if ($data['trash'])
{
    ...
    echo "        <tr>\n";
    echo "            <th>&nbsp;</th>\n";
    echo "            <th>" . midcom::i18n()->get_string('title', 'midcom') . "</th>\n";
    echo "            <th>" . midcom::i18n()->get_string('deleted on', 'midgard.admin.asgard') . "</th>\n";
    echo "            <th>" . midcom::i18n()->get_string('deleted by', 'midgard.admin.asgard') . "</th>\n";
    echo "            <th>" . midcom::i18n()->get_string('size', 'midgard.admin.asgard') . "</th>\n";
    echo "        </tr>\n";
    echo "    </thead>\n";
    echo "    <tfoot>\n";
    echo "            <tr>\n";
    ...
    echo "        <tr>\n";
    echo "            <td colspan=\"5\">\n";
    echo "                <input type=\"submit\" value=\"" . midcom::i18n()->get_string('undelete', 'midgard.admin.asgard') . "\" />\n";
    echo "                <input type=\"submit\" name=\"purge\" value=\"" . midcom::i18n()->get_string('purge', 'midgard.admin.asgard') . "\" />\n";
    echo "            </td>\n";
    echo "        </tr>\n";
    echo "    </tfoot>\n";
    echo "    <tbody>\n";
...
{
    echo "<p>" . midcom::i18n()->get_string('trash is empty', 'midgard.admin.asgard') . "</p>\n";
}
?>