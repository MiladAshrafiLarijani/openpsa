...
$type_choices = Array('any' => midcom::i18n()->get_string('any', 'midgard.admin.asgard')) + $type_choices;

$revised_after_choices = array();
if ($data['config']->get('enable_review_dates'))
{
    $review_by_choices = array();
    $revised_after_choices['any'] = midcom::i18n()->get_string('any', 'midgard.admin.asgard');
    $review_by_choices['any'] = midcom::i18n()->get_string('any', 'midgard.admin.asgard');
    // 1 week
    $date = mktime(0, 0, 0, date('m'), date('d') + 6, date('Y'));
    $review_by_choices[$date] = midcom::i18n()->get_string('1 week', 'midgard.admin.asgard');
    // 2 weeks
    $date = mktime(0, 0, 0, date('m'), date('d') + 13, date('Y'));
    $review_by_choices[$date] = midcom::i18n()->get_string('2 weeks', 'midgard.admin.asgard');
    // 1 month
    $date = mktime(0, 0, 0, date('m') + 1, date('d'), date('Y'));
    $review_by_choices[$date] = midcom::i18n()->get_string('1 month', 'midgard.admin.asgard');
}

// 1 day
$date = mktime(0, 0, 0, date('m'), date('d') - 1, date('Y'));
$revised_after_choices[$date] = midcom::i18n()->get_string('1 day', 'midgard.admin.asgard');
// 1 week
$date = mktime(0, 0, 0, date('m'), date('d') - 6, date('Y'));
$revised_after_choices[$date] = midcom::i18n()->get_string('1 week', 'midgard.admin.asgard');
// 1 month
$date = mktime(0, 0, 0, date('m') - 1, date('d'), date('Y'));
$revised_after_choices[$date] = midcom::i18n()->get_string('1 month', 'midgard.admin.asgard');
?>
    ...
    <div class="filter">
        <form name="latest_objects_filter" method="get">
            <div class="type_filter">
                <label for="type_filter"><?php echo midcom::i18n()->get_string('type', 'midgard.admin.asgard'); ?></label>
            ...
            <div class="revised_after">
                <label for="revised_after"><?php echo midcom::i18n()->get_string('objects revised within', 'midgard.admin.asgard'); ?></label>
            ...
            if ($data['config']->get('enable_review_dates'))
            {
                ?>
            <div class="review_by">
                <label for="review_by"><?php echo midcom::i18n()->get_string('objects expiring within', 'midgard.admin.asgard'); ?></label>
            ...
            <label for="only_mine">
                <?php echo midcom::i18n()->get_string('only mine', 'midgard.admin.asgard'); ?></label>
            </label>
            <input type="submit" name="filter" value="<?php echo midcom::i18n()->get_string('filter', 'midgard.admin.asgard'); ?>" />
        </form>
    </div>
...
if (count($data['revised']) > 0)
{
    ...
    echo "            <th class=\"title\">" . midcom::i18n()->get_string('title', 'midcom') . "</th>\n";

    if ($data['config']->get('enable_review_dates'))
    {
        echo "            <th class=\"review_by\">" . midcom::i18n()->get_string('review date', 'midgard.admin.asgard') . "</th>\n";
    }

    echo "            <th class=\"revised\">" . midcom::i18n()->get_string('revised', 'midcom.admin.folder') . "</th>\n";
    echo "            <th class=\"revisor\">" . midcom::i18n()->get_string('revisor', 'midcom.admin.folder') . "</th>\n";
    echo "            <th class=\"approved\">" . midcom::i18n()->get_string('approved', 'midcom') . "</th>\n";
    echo "            <th class=\"revision\">" . midcom::i18n()->get_string('revision', 'midcom.admin.folder') . "</th>\n";
    echo "        </tr>\n";
    echo "    </thead>\n";
    echo "    <tfoot>\n";
    echo "            <tr>\n";
    ...
    echo "                    <input type=\"checkbox\" name=\"select_all\" id=\"select_all\" value=\"\" onclick=\"jQuery(this).check_all('#batch_process tbody');\" />" . midcom::i18n()->get_string('select all', 'midgard.admin.asgard');
    echo "                </label>\n";
    echo "                <label for=\"invert_selection\">\n";
    echo "                    <input type=\"checkbox\" name=\"invert_selection\" id=\"invert_selection\" value=\"\" onclick=\"jQuery(this).invert_selection('#batch_process tbody');\" />" . midcom::i18n()->get_string('invert selection', 'midgard.admin.asgard');
    echo "                </label>\n";
    echo "            </td>\n";
    echo "        </tr>\n";
    echo "    </tfoot>\n";
    ...
    foreach ($data['revised'] as $object)
    {
        ...
        {
            $approved_str = midcom::i18n()->get_string('not approved', 'midgard.admin.asgard');
        }
        $title = substr($data['reflectors'][$class]->get_object_label($object), 0, 60);
        if (empty($title))
        {
            $title = '[' . midcom::i18n()->get_string('no title', 'midgard.admin.asgard') . ']';
        }

        if (!isset($revisors[$object->metadata->revisor]))
        {
            $revisors[$object->metadata->revisor] = midcom::auth->get_user($object->metadata->revisor);
        }
        ...
        <div class="actions">
            <div class="action">
                <select name="mass_action" id="mass_action">
                    <option value=""><?php echo midcom::i18n()->get_string('choose action', 'midgard.admin.asgard'); ?></option>
                    <option value="delete"><?php echo midcom::i18n()->get_string('delete', 'midcom'); ?></option>
                    <option value="approve"><?php echo midcom::i18n()->get_string('approve', 'midcom'); ?></option>
                </select>
            </div>
            <input type="submit" name="execute_mass_action" value="<?php echo midcom::i18n()->get_string('apply to selected', 'midgard.admin.asgard'); ?>" />
        </div>
    </form>
...
else
{
    ...
    if (count($activities) > 0)
    {
        ...
        echo "<h2>" . midcom::i18n()->get_string('latest activities', 'midcom.helper.activitystream') . "</h2>\n";
        echo "<table class=\"results table_widget\" id =\"last_visited\">\n";
        echo "    <thead>\n";
        echo "        <tr>\n";
        echo "            <th class=\"icon\">&nbsp;</th>\n";
        echo "            <th class=\"title\">" . midcom::i18n()->get_string('title', 'midcom') . "</th>\n";      
        echo "            <th class=\"revised\">" . midcom::i18n()->get_string('date', 'midcom') . "</th>\n";
        echo "            <th class=\"revisor\">" . midcom::i18n()->get_string('actor', 'midcom.helper.activitystream') . "</th>\n";
        echo "            <th class=\"action\">" . midcom::i18n()->get_string('action', 'midcom.helper.activitystream') . "</th>\n";
        echo "        </tr>\n";
        echo "    </thead>\n";
        echo "    <tbody>\n";
        foreach ($activities as $activity)
        {
            $object = midcom::dbfactory()->get_object_by_guid($activity->target);
            if (   !$object
                || empty($object->guid))
            {
