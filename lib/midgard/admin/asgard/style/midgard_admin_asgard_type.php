...
{
    $component_label = midcom::i18n()->get_string($data['component'], $data['component']);
}
echo "<h2>";
echo sprintf(midcom::i18n()->get_string('%s in %s', 'midcom'),
...
if ($data['parent_type'])
{
    echo sprintf(midcom::i18n()->get_string('%s is under type %s', 'midgard.admin.asgard'), midgard_admin_asgard_plugin::get_type_label($data['type']), "<a href=\"{$prefix}__mfa/asgard/{$data['parent_type']}/\">" . midgard_admin_asgard_plugin::get_type_label($data['parent_type']) . "</a>") . ' ';
}

if ($data['component'] == 'midgard')
{
    echo midcom::i18n()->get_string('this is a midgard core type', 'midgard.admin.asgard') . "</p>\n";
}
else
{
    echo sprintf(midcom::i18n()->get_string('this type belongs to %s component', 'midgard.admin.asgard'), "<a href=\"{$prefix}__mfa/asgard/components/{$data['component']}/\">{$data['component']}</a>") . "</p>\n";
}
?>
...
<div id="search_bar">
    <label>
        <?php echo midcom::i18n()->get_string('search', 'midgard.admin.asgard'); ?>
        <input id="search_field" type="text" name="search" class="search"<?php if (isset($_GET['search'])) { echo " value=\"{$_GET['search']}\""; } ?> />
    </label>
    <input class="search" type="submit" value="<?php echo midcom::i18n()->get_string('go', 'midgard.admin.asgard'); ?>" />
</div>
</form>
...
if (isset($data['search_results']))
{
    ...
    {
        echo "<p>" . midcom::i18n()->get_string('no results', 'midgard.admin.asgard') . "</p>\n";
    }
    ...
    else
    {
        ...
        echo "            <th>" . midcom::i18n()->get_string('created on', 'midgard.admin.asgard') . "</th>\n";
        echo "            <th>" . midcom::i18n()->get_string('created by', 'midgard.admin.asgard') . "</th>\n";
        echo "        </tr>\n";
        echo "    </thead>\n";
        echo "    <tbody>\n";
        $persons = array();
        foreach ($data['search_results'] as $result)
        {
            ...
            {
                $persons[$result->metadata->creator] = midcom::auth->get_user($result->metadata->creator);
            }
