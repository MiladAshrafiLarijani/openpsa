<form method="get" class="midcom_admin_user_search">
    <label>
        <span><?php echo midcom::i18n()->get_string('search person', 'midcom.admin.user'); ?></span>
    ...
    <input type="submit" value="<?php echo midcom::i18n()->get_string('go', 'midcom.admin.user'); ?>" />
    <div class="helptext">
        <?php
        $data['search_fields_l10n'] = array();
        foreach ($data['search_fields'] as $field)
        {
            $data['search_fields_l10n'][] = midcom::i18n()->get_string($field, 'midcom.admin.user');
        }
        echo sprintf(midcom::i18n()->get_string('the following fields will be searched: %s', 'midcom.admin.user'), implode(', ', $data['search_fields_l10n']));
        ?>
    </div>
</form>
    ...
    <table class="midcom_admin_user_search_results">
        <thead>
            <tr>
                ...
                {
                    echo '<th>' . midcom::i18n()->get_string($field, 'midcom.admin.user') . "</th>\n";
                }
                ?>
                <th><?php echo midcom::i18n()->get_string('groups', 'midcom.admin.user'); ?></th>
            </tr>
        </thead>
        <tbody>
    <?php
