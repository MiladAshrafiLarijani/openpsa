...
class midcom_services_auth_acl
{
    ...
    function get_privileges_by_guid($object_guid, $object_class, $user_id)
    {
        ...
        if (!array_key_exists($cache_id, self::$_privileges_cache))
        {
            ...
            {
                midcom::generate_error(MIDCOM_ERRCRIT, "class '{$object_class}' does not exist");
                // This will exit()
            }
    ...
    private static function _collect_content_privileges($guid, $user_id, $class = null)
    {
        ...
        // ==> into SUDO
        $previous_sudo = midcom::auth->acl->_internal_sudo;
        midcom::auth->acl->_internal_sudo = true;

        $parent_guid = midcom::dbfactory()->get_parent_guid($guid, $class);

        midcom::auth->acl->_internal_sudo = $previous_sudo;
        // <== out of SUDO
        ...
        $valid_privileges[MIDCOM_PRIVILEGE_SCOPE_OWNER] = midcom::auth->acl->get_owner_default_privileges();
    ...
    function can_do_byclass($privilege, $user, $class, $component)
    {
        ...
        if ($full_privileges['midgard:owner'] == MIDCOM_PRIVILEGE_ALLOW)
        {
            ...
            (
                $full_privileges,
                midcom::auth->get_owner_default_privileges()
            );
        }
    ...
    private static function _load_content_privilege($privilegename, $guid, $class, $user_id)
    {
        ...
        {
            $owner_privileges = midcom::auth->acl->get_owner_default_privileges();
