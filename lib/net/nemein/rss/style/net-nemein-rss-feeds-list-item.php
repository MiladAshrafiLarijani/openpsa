...
switch ($data['topic']->component)
{
    case 'net.nehmer.blog':
        ...
        echo "        <li><a href=\"{$prefix}category/{$data['feed_category']}/\">" . sprintf(midcom::i18n()->get_string('%s items', 'net.nemein.rss'), $data['feed_items']) . "</a></li>\n";
        break;
}

if ($data['feed']->latestupdate)
{
    echo "        <li>" . sprintf(midcom::i18n()->get_string('latest item from %s', 'net.nemein.rss'), strftime('%x %X', $data['feed']->latestupdate)) . "</li>\n";
}
if ($data['feed']->latestfetch)
{
    echo "        <li>" . sprintf(midcom::i18n()->get_string('latest fetch %s', 'net.nemein.rss'), strftime('%x %X', $data['feed']->latestfetch)) . "</li>\n";
}
echo "    </ul>\n";
...
if ($data['topic']->can_do('midgard:create'))
{
    ...
    (
        ...
        (
            ...
            MIDCOM_TOOLBAR_LABEL => midcom::i18n()->get_string('refresh feed', 'net.nemein.rss'),
...
if ($data['feed']->can_do('midgard:delete'))
{
    ...
    (
        ...
        (
            ...
            MIDCOM_TOOLBAR_LABEL => midcom::i18n()->get_string('delete feed', 'net.nemein.rss'),
