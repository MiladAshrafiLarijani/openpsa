...
<li class="document" id="org_openpsa_relatedto_line_&(link['guid']);">
  ...
  <ul class="metadata">
    ...
    if (count($atts) == 0)
    {
        echo midcom::i18n()->get_string('no files', 'org.openpsa.documents');
    }
    else
    {
        foreach ($atts as $file)
        {
            ...
            {
                echo "<a target=\"document_{$document->guid}\" href=\"" . midcom_connection::get_url('self') . "midcom-serveattachmentguid-{$file->guid}/{$file->name}\">{$file->name}</a> (" . sprintf(midcom::i18n()->get_string('%s document', 'org.openpsa.documents'), midcom::i18n()->get_string($file->mimetype, 'org.openpsa.documents')).")";
            }
        }
    }
    ?>
