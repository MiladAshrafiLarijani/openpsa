...
if ($invoice->paid > 0)
{
    $paid = ", " . midcom::i18n()->get_string('paid', 'org.openpsa.invoices') . ": " . strftime('%x', $invoice->paid);
}
else if ($invoice->due < time())
{
    $paid = ", " . midcom::i18n()->get_string('not paid', 'org.openpsa.invoices');
}
?>
...
<li class="invoice" id="org_openpsa_relatedto_line_&(link['guid']);">
    ...
    if ($invoice->customer)
    {
        ...
        echo "<li>" . midcom::i18n()->get_string('customer', 'org.openpsa.invoices') . ": {$customer->official}</li>";
    }

    echo "<li>" . midcom::i18n()->get_string('sum', 'org.openpsa.invoices') . ": " . org_openpsa_helpers::format_number($invoice->sum) . " (" . midcom::i18n()->get_string('due', 'org.openpsa.invoices') . ": " . strftime('%x', $invoice->due) . "{$paid})</li>";
    ?>
    </ul>
