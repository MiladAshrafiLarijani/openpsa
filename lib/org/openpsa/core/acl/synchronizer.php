...
class org_openpsa_core_acl_synchronizer
{
    ...
    function _write_full_midcom_acls($object, $owner_id, $accesstype)
    {
        ...
        $owner_object = midcom::auth->get_assignee($owner_id);
        if (!$owner_object
        ...
        if ($privileges)
        {
            ...
            {
                if (   $privilege->assignee != midcom::auth->user->id
        ...
        switch ($accesstype)
        {
            ...
            case ORG_OPENPSA_ACCESSTYPE_PRIVATE:
                ...
                $object->set_privilege('midgard:owner', midcom::auth->user->id, MIDCOM_PRIVILEGE_ALLOW);
                $this->_set_attachment_permission($object, 'midgard:read', midcom::auth->user->id, MIDCOM_PRIVILEGE_ALLOW);
                break;
            ...
            case ORG_OPENPSA_ACCESSTYPE_WGRESTRICTED:
                ...
                $subscriber_group = midcom::auth->get_group($owner_id.'subscribers');
                if ($subscriber_group
